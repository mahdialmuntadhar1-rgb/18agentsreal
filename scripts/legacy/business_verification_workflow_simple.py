#!/usr/bin/env python3
"""
Local Business Verification Workflow - Simple Version (No pandas dependency)
...
class BusinessVerificationWorkflowSimple:
    ...
    def read_xlsx_file_simple(self, file_path: Path) -> Tuple[List[str], List[Dict]]:
        """Simple XLSX reader - for now just skip XLSX files"""
    ...
    def process_file(self, file_path: Path) -> List[Dict]:
        ...
        if file_path.suffix.lower() == '.csv':
            ...
            headers, data = self.read_xlsx_file_simple(file_path)
        else:
    ...
    def verify_business_online(self, business: Dict) -> Dict:
        ...
        if len(name) > 2:
            sources_tried.append('name_valid')
        
        # 2. Phone verification (if phone exists)
        if phone and phone != '':
            if business.get('phone_status') == 'valid':
                sources_tried.append('phone_format_valid')
        
        # 3. Location verification
        if city or governorate:
            sources_tried.append('location_present')
        
        # 4. Additional data points
        if business.get('website') or business.get('facebook') or business.get('instagram'):
            sources_tried.append('social_media_present')
        
        # Set confidence based on sources found
        if len(sources_tried) >= 3:
            verification_result['verification_confidence'] = 'high'
        elif len(sources_tried) >= 2:
    ...
    def categorize_business(self, business: Dict) -> str:
        ...
        category_mapping = {
            'restaurant': ['restaurant', 'مطعم', 'طعام', 'food', 'كباب', 'وجبة'],
            'cafe': ['cafe', 'مقهى', 'قهوة', 'coffee', 'كافي'],
            'hotel': ['hotel', 'فندق', 'hotel'],
            'pharmacy': ['pharmacy', 'صيدلية', 'دواء'],
            'shop': ['shop', 'متجر', 'store', 'shop', 'سوق', 'market'],
            'electronics': ['electronics', 'إلكترونيات'],
            'clothing': ['clothing', 'ملابس', 'clothes'],
            'furniture': ['furniture', 'أثاث'],
            'education': ['school', 'مدرسة', 'education', 'university', 'academy'],
            'services': ['services', 'خدمات'],
            'fuel': ['fuel', 'وقود', 'gas station'],
            'mosque': ['mosque', 'مسجد'],
    ...
    def generate_output_files(self, verified_with_phone: List[Dict], verified_without_phone: List[Dict], 
        ...
        # A. Verified businesses with phone (CSV)
        if verified_with_phone:
            self.save_csv(verified_with_phone, f"verified_businesses_{timestamp}.csv")
        
        # B. Verified businesses without phone (CSV)
        if verified_without_phone:
            self.save_csv(verified_without_phone, f"businesses_without_phone_{timestamp}.csv")
        
        # C. Businesses needing review (CSV)
        if needs_review:
            self.save_csv(needs_review, f"businesses_needing_review_{timestamp}.csv")
        
        # D. Duplicate report (CSV)
        if duplicate_report:
            self.save_csv(duplicate_report, f"duplicate_report_{timestamp}.csv")
...
Verification Sources: Name validation, Phone format, Location data, Social media

OUTPUT FILES GENERATED:
---------------------
- verified_businesses_[timestamp].csv (verified with valid phone)
- businesses_without_phone_[timestamp].csv (verified but no phone)
- businesses_needing_review_[timestamp].csv (ambiguous/low confidence)
- duplicate_report_[timestamp].csv (duplicate removal report)

NEXT STEPS:
----------
1. Review verified_businesses.csv for high-confidence verified data
2. Check businesses_without_phone.csv for real businesses needing phone numbers
3. Review businesses_needing_review.csv for manual verification
4. Use duplicate_report.csv to understand deduplication decisions
...
def main():
    ...
    workflow = BusinessVerificationWorkflowSimple(input_folder, output_folder)
