  ...
  const columns: Column<AgentJob>[] = [
    {
      header: 'Agent ID',
      sortKey: 'agentName',
      accessor: (job: AgentJob) => <span className="font-mono text-xs font-bold text-slate-900">{job.agentName}</span>
    },
    {
    ...
    },
    {
      ...
      accessor: (job: AgentJob) => <div className="text-center"><StatusBadge status={job.status} /></div>
    },
    {
      header: 'Records',
      sortKey: 'recordsFound',
      accessor: (job: AgentJob) => <span className="text-sm font-mono text-slate-600">{job.recordsFound.toLocaleString()}</span>
    },
    {
      header: 'Errors',
      sortKey: 'errorCount',
